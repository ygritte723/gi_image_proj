...
def setup_run(arg_mode="train"):
    ...
    args.save_path = os.path.join(
        f"checkpoints/{args.dataset}/{args.shot}shot-{args.way}way/", args.extra_dir
    )
    ...
    if not args.no_wandb:
        ...
        wandb.init(
            project=f"renet-{args.dataset}-{args.way}w{args.shot}s",
            config=args,
            save_code=True,
            name=args.extra_dir,
        )
...
def compute_accuracy(logits, labels):
    ...
    return (pred == labels).type(torch.float).mean().item() * 100.0
...
def parse_args(arg_mode):
    parser = argparse.ArgumentParser(
        description="Relational Embedding for Few-Shot Classification (ICCV 2021)"
    )
    ...
    parser.add_argument(
        "-data_dir",
        default="/jet/home/lisun/work/xinliu/images/fewshot",
    ...
    parser.add_argument(
        "-lamb", type=float, default=0.25, metavar="lambda", help="loss balancing term"
    )
    ...
    parser.add_argument(
        "-gamma", type=float, default=0.05, help="learning rate decay factor"
    )
    parser.add_argument(
        "-milestones",
        nargs="+",
    ...
    parser.add_argument(
        "-save_all", action="store_true", help="save models on each epoch"
    )

    """ about few-shot episodes """
    parser.add_argument(
        "-way", type=int, default=2, metavar="N", help="number of few-shot classes"
    )
    parser.add_argument(
        "-shot", type=int, default=5, metavar="K", help="number of shots"
    )
    parser.add_argument(
        "-query", type=int, default=15, help="number of query image per class"
    )
    parser.add_argument(
        "-val_episode", type=int, default=200, help="number of validation episode"
    )
    parser.add_argument(
        "-test_episode",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="resnet12",
        choices=["resnet12", "conv4", "transformer"],
    )
    ...
    """ about env """
    parser.add_argument(
        "-gpu", default="0", help='the GPU ids e.g. "0", "0,1", "0,1,2", etc'
    )
    parser.add_argument(
        "-extra_dir",
        type=str,
    ...
    parser.add_argument(
        "-no_wandb",
        action="store_true",
        help="not plotting learning curve on wandb",
        default=arg_mode == "train",
    )  # train: enable logging / test: disable logging
    parser.add_argument(
        "-resume", type=str, default="", help="resume from this checkpoint"
    )
