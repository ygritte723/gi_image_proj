...
def generate_spatial_descriptor(data, kernel_size):
    ...
    assert data_padded.shape == (
        ...
        (h + 2 * padding),
        (w + 2 * padding),
    ), "Error: data_padded shape{} wrong!".format(data_padded.shape)

    output = torch.zeros(
        size=[b, kernel_size * kernel_size, h, w], requires_grad=data.requires_grad
    )
    if data.is_cuda:
...
class SpatialContextEncoder(torch.nn.Module):
    ...
    def __init__(self, planes=None, kernel_size=None):
        ...
        self.embeddingFea = nn.Sequential(
            nn.Conv2d(
                planes[1] + kernel_size * kernel_size,
                planes[2],
                kernel_size=1,
                bias=False,
                padding=0,
        ...
        print(
            "SpatialContextEncoder initialization: input_dim {},hidden_dim {}".format(
                planes[1], planes[2]
            )
        )
        return
