    ...
    for i, ((data, train_labels), (data_aux, train_labels_aux)) in enumerate(
        ...
        if args.mix:
            ...
            if i + 1 < len(tqdm_gen):
                (i + 1) % len(tqdm_gen)
                (
                    (data_sec, train_labels_sec),
                    (data_aux_sec, train_labels_aux_sec),
                ) = next(zip(train_loader, train_loader_aux), 1)
                data_sec, train_labels_sec = data_sec.cuda(), train_labels_sec.cuda()
                data_aux_sec, train_labels_aux_sec = (
                    data_aux_sec.cuda(),
                    train_labels_aux_sec.cuda(),
                )
        ...
        data_aux = model(
            data_aux
        )  # I prefer to separate feed-forwarding data and data_aux due to BN
        ...
        logits, absolute_logits = model(
            (data_shot.unsqueeze(0).repeat(args.num_gpu, 1, 1, 1, 1), data_query)
        )
...
def train_main(args):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if train_acc > max_acc:
            ...
            print(
                f"[ log ] *********A better model is found ({train_acc:.3f}) *********"
            )

            max_acc, max_epoch = train_acc, epoch
            torch.save(
        ...
        if args.save_all:
            ...
            torch.save(
                optimizer.state_dict(),
                os.path.join(args.save_path, f"optimizer_epoch_{epoch}.pth"),
            )

        epoch_time = time.time() - start_time
        print(f"[ log ] saving @ {args.save_path}")
        print(
            f"[ log ] roughly {(args.max_epoch - epoch) / 3600. * epoch_time:.2f} h left\n"
        )
