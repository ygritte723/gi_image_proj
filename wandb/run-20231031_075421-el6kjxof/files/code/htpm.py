...
padim = (
    lambda x, h_max: np.concatenate(
        (x, x.view(-1)[0].copy().expand(1, 3, h_max - x.shape[2], x.shape[3]) / 1e20),
        axis=0,
    )
    if x.shape[2] < h_max
    else x
...
def attn_heatmap_cca(img_s, img_q, attn_s_qs, attn_q_qs):
    ...
    attn_s_qs_normalized = (attn_s_qs - attn_s_qs.min()) / (
        attn_s_qs.max() - attn_s_qs.min()
    )
    attn_q_qs_normalized = (attn_q_qs - attn_q_qs.min()) / (
        attn_q_qs.max() - attn_q_qs.min()
    )
    ...
    im = np.concatenate(
        (padim(img_s_heatmap, h_max), padim(img_q_heatmap, h_max)), axis=1
    )
...
if __name__ == "__main__":
    # load data
    img_s_path = "/jet/home/lisun/work/xinliu/images/fewshot/kvasirv2/polyps/00072d5f-7cd8-434c-8a5a-1a0bb2c9711d.jpg"
    img_q_path = "/jet/home/lisun/work/xinliu/images/fewshot/kvasirv2/ulcerative-colitis/000eeefc-9035-4d42-92a1-2f97a3200ee0.jpg"
    ...
    attn_s = F.interpolate(
        attn_s, size=(84, 84), mode="bilinear", align_corners=False
    ).squeeze()
    # Resize
    attn_q = F.interpolate(
        attn_q, size=(84, 84), mode="bilinear", align_corners=False
    ).squeeze()
    # Convert back to original format

    feature_img_s = torch.squeeze(feature_img_s).mean(dim=0)
    feature_img_s = F.interpolate(
        feature_img_s.unsqueeze(0).unsqueeze(0),
        size=(84, 84),
        mode="bilinear",
        align_corners=False,
    ).squeeze()
    feature_img_q = torch.squeeze(feature_img_q).mean(dim=0)
    feature_img_q = F.interpolate(
        feature_img_q.unsqueeze(0).unsqueeze(0),
        size=(84, 84),
        mode="bilinear",
        align_corners=False,
    ).squeeze()
