...
def attn_ori(img_s, img_q):
    ...
    img_s = img_s.resize(
        (int(img_s.size[0] * h_max / img_s.size[1]), h_max), Image.ANTIALIAS
    )
    img_q = img_q.resize(
        (int(img_q.size[0] * h_max / img_q.size[1]), h_max), Image.ANTIALIAS
    )
...
if __name__ == "__main__":
    ...
    attn_ori(
        Image.open(img_s_path).convert("RGB"), Image.open(img_q_path).convert("RGB")
    )
    img_s = img_s_bs.cuda()
    img_q = img_q_bs.cuda()
    # print(img_s.size())
    ...
    attn_s = F.interpolate(
        attn_s, size=(image_size, image_size), mode="bilinear", align_corners=False
    ).squeeze()
    # Resize
    attn_q = F.interpolate(
        attn_q, size=(image_size, image_size), mode="bilinear", align_corners=False
    ).squeeze()
