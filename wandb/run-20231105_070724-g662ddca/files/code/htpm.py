...
if __name__ == '__main__':
    ...
    # Resize
    attn_s = F.interpolate(attn_s, size=(image_size, image_size), mode='bilinear', align_corners=False).squeeze()
    # Resize
    attn_q = F.interpolate(attn_q, size=(image_size, image_size), mode='bilinear', align_corners=False).squeeze()
    # Convert back to original format
    
    feature_img_s = torch.squeeze(feature_img_s).mean(dim=0)
    feature_img_s = F.interpolate(feature_img_s.unsqueeze(0).unsqueeze(0), size=(image_size, image_size), mode='bilinear', align_corners=False).squeeze()
    feature_img_q = torch.squeeze(feature_img_q).mean(dim=0)
    feature_img_q = F.interpolate(feature_img_q.unsqueeze(0).unsqueeze(0), size=(image_size, image_size), mode='bilinear', align_corners=False).squeeze()
