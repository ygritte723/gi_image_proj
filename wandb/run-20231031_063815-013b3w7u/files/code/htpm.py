...
if __name__ == "__main__":
    ...
    # Reshape for PyTorch
    attn_s = attn_s.permute(2, 0, 1).unsqueeze(0)
    # Resize
    attn_s = F.interpolate(attn_s, size=(84, 84), mode="bilinear", align_corners=False)
    # Convert back to original format
    attn_s = attn_s.squeeze(0).permute(1, 2, 0)
    # Reshape for PyTorch
    attn_q = attn_q.permute(2, 0, 1).unsqueeze(0)
    # Resize
    attn_q = F.interpolate(attn_q, size=(84, 84), mode="bilinear", align_corners=False)
    # Convert back to original format
    attn_q = attn_q.squeeze(0).permute(1, 2, 0)
    # draw cam
