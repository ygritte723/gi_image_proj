...
if __name__ == "__main__":
    ...
    img_s = cv2.resize(
        img_s, (int(img_s.shape[0] * h_max / img_s.shape[1]), h_max), cv2.INTER_CUBIC
    )
    img_q = cv2.resize(
        img_q, (int(img_q.shape[0] * h_max / img_q.shape[1]), h_max), cv2.INTER_CUBIC
    )
    ...
    for cams1 in cams:
        ...
        for path in paths:
            ...
            input_tensor = (
                torchvision.transforms.functional.to_tensor(rgb_img)
                .unsqueeze(0)
                .float()
            )
        ...
        img_s_att = cv2.resize(
            img_s_att,
            (int(img_s_att.shape[0] * h_max / img_s_att.shape[1]), h_max),
            cv2.INTER_AREA,
        )
        img_q_att = cv2.resize(
            img_q_att,
            (int(img_q_att.shape[0] * h_max / img_q_att.shape[1]), h_max),
            cv2.INTER_AREA,
        )
